...
- Angular language ownership is split to avoid duplicate TypeScript LSP results: `typescript-tools.nvim` fully owns JavaScript/TypeScript/TSX buffers (referencing via its tsserver + Angular tsserver plugins), while standalone `ngserver` owns every Angular template buffer (both the stock `html` filetype and the `htmlangular` marker) and never attaches to `typescript`/`typescriptreact`. ngserver's `on_attach` additionally neuters any vanilla HTML LSP that attaches to `.html` so Angular is the sole template provider.
