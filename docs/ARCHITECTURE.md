...
- Avante.nvim handoff alternative: `:OpenCodeReviewAvante` (`<leader>ov`) reuses the same comment collection and `.omo/review-*.json` writing, then hands off to avante.nvim via `avante.api.ask({ question = ... })` instead of the opencode terminal. Avante is packaged from nixpkgs (`vimPlugins.avante-nvim`) in `config/editor/avante.nix`, lazy-loaded on its commands only (no startup cost, no startup API-key prompt); nvf's `lzn-auto-require` auto-loads it when the review module requires `avante.api`. On handoff failure the review session stays alive so `:OpenCodeReviewComplete` remains a fallback.
- Avante.nvim model + instruction parity: default provider is GitHub Copilot with model `gpt-5.6-terra`; auth reuses the existing `~/.config/github-copilot/{hosts,apps}.json` OAuth token (no copilot.lua dependency). To mirror opencode's always-on instruction set, `config/editor/avante.nix` builds an `agentic.avanterules` rules file from the same repo sources opencode deploys (`AGENTS.md`, `.opencode/skills/*/SKILL.md`) and points avante's `rules.global_dir` at it — avante injects this into every agentic-mode request. Adding a new skill under `.opencode/skills/<name>/SKILL.md` automatically includes it for both agents.
